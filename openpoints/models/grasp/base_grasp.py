...
class BaseGrasp(nn.Module):
    ...
    def forward(self, data):
        p, f, ids = self.encoder.forward_grasp_feat(data)
        if self.decoder is not None:
            f = self.decoder(p, f)
        if self.head is not None:
            f = self.head(f)
        return p[-4:], f[-4:], ids[-4:]


@MODELS.register_module()
class BasePartGrasp(BaseGrasp):
    ...
    def forward(self, p0, f0=None, cls0=None):
        ...
        p, f = self.encoder.forward_grasp_feat(p0, f0)
        if self.decoder is not None:
...
class VariableGrasp(BaseGrasp):
    ...
    def forward(self, data):
        p, f, b = self.encoder.forward_grasp_feat(data)
...
class GraspHead(nn.Module):
    def __init__(self,
        ...
        """A scene grasp head for ResNet backbone.
        Args:
...
@MODELS.register_module()
class VariableGraspHead(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
...
@MODELS.register_module()
class MultiGraspHead(nn.Module):
    def __init__(self,
                 num_classes, in_channels,
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
...
#     def forward(self, data):
#         p, f = self.encoder.forward_grasp_feat(data)
#         if self.decoder is not None:
