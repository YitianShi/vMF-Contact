...
from ...cpp.pointops.functions import pointops
...
class TransitionDown(nn.Module):
    ...
    def forward(self, feats, xyz, offset):
        ...
        feats = pointops.queryandgroup(self.k, xyz, n_xyz, feats, None, offset, n_offset,
                                       use_xyz=False)  # (m, nsample, 3+c)
        m, k, c = feats.shape
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, quant_size, rel_query=True, rel_key=False, rel_value=False,
    ...
    def forward(self, feats, xyz, index_0, index_1, index_0_offsets, n_max):
        ...
        if self.rel_query and self.rel_key:
            relative_position_bias = pointops.dot_prod_with_idx_v3(query.float(), index_0_offsets.int(), n_max,
                                                                   key.float(), index_1.int(),
                                                                   self.relative_pos_query_table.float(),
                                                                   self.relative_pos_key_table.float(),
                                                                   relative_position_index.int())
        elif self.rel_query:
            relative_position_bias = pointops.dot_prod_with_idx(query.float(), index_0.int(),
                                                                self.relative_pos_query_table.float(),
                                                                relative_position_index.int())  # [M, num_heads]
        elif self.rel_key:
            relative_position_bias = pointops.dot_prod_with_idx(key.float(), index_1.int(),
                                                                self.relative_pos_key_table.float(),
                                                                relative_position_index.int())  # [M, num_heads]
        else:
        ...
        if self.rel_value:
            x = pointops.attention_step2_with_rel_pos_value_v2(softmax_attn_flat.float(), value.float(),
                                                               index_0_offsets.int(), n_max, index_1.int(),
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size, quant_size,
                 ...
                 mlp_ratio=4.0, qkv_bias=True, qk_scale=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()

        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention(dim, window_size, num_heads=num_heads, quant_size=quant_size,
                                    rel_query=rel_query, rel_key=rel_key, rel_value=rel_value, qkv_bias=qkv_bias,
                                    qk_scale=qk_scale)
    ...
    def forward(self, feats, xyz, index_0, index_1, index_0_offsets, n_max):
        ...
        # this blocks follows the same as Standard Transformer
        short_cut = feats
...
class BasicLayer(nn.Module):
    def __init__(self, downsample_scale, depth, channel, num_heads, window_size, grid_size, quant_size,
        ...
        self.blocks = nn.ModuleList([SwinTransformerBlock(channel, num_heads, window_size, quant_size,
                                                          rel_query=rel_query, rel_key=rel_key, rel_value=rel_value,
    ...
    def forward(self, feats, xyz, offset):
        ...
        shift_new_v2p_map, shift_new_p2v_map, shift_new_counts = grid_sample(xyz + shift_size, batch, new_window_size,
                                                                             start=xyz.min(0)[0])
        ...
        for i, blk in enumerate(self.blocks):
            ...
            index_0, index_1 = get_indice_pairs(p2v_map_blk, counts_blk, new_p2v_map_blk, new_counts_blk,
                                                downsample_idx, batch, xyz, window_size, i)
...
class Upsample(nn.Module):
    ...
    def forward(self, feats, xyz, support_xyz, offset, support_offset, support_feats=None):
        feats = self.linear1(support_feats) + pointops.interpolation(xyz, support_xyz, self.linear2(feats), offset,
                                                                     support_offset)
...
class StratifiedEncoder(nn.Module):
    def __init__(self, downsample_scale, depths, channels, num_heads, window_size, \
                 patch_size, grid_size, quant_size, rel_query=True, rel_key=False, rel_value=False, drop_path_rate=0.2, \
                 num_layers=4, in_channels=3, num_classes=13, ratio=0.25, k=16, prev_grid_size=0.04, sigma=1.0,
                 stem_transformer=False, **kwargs):
        super().__init__()

        # stocahstics depths training is useful for training larger models
        ...
        self.layers = nn.ModuleList(
            [BasicLayer(downsample_scale, depths[i], channels[i], num_heads[i], window_size[i], grid_sizes[i], \
                        ...
                        ratio=ratio, k=k, out_channels=channels[i + 1] if i < num_layers - 1 else None) for i in
    ...
    def forward_cls_features(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # If it isn't the last layer, feats, xyz, offset store the downsampled layers,
            # else in the last layer, store the original layers
    ...
    def forward_seg_feat(self, xyz, feats=None, offset=None, batch=None, neighbor_idx=None):
        if hasattr(xyz, 'keys'): 
            xyz, feats, offset, batch, neighbor_idx = xyz['pos'], xyz['x'], xyz['o'], xyz.get('batch', None), xyz.get('neighbor_idx', None)
        
        if neighbor_idx is None:
            radius = 2.5 * self.grid_size * self.sigma
            neighbor_idx = tp.ball_query(radius, self.max_num_neighbors, xyz, xyz, mode="partial_dense", batch_x=batch, batch_y=batch)[0]
        feats_stack = []
        xyz_stack = []
        offset_stack = []
        ...
        feats = feats.contiguous()  # why this is necessary?
        ...
        for i, layer in enumerate(self.layers):
            ...
            xyz = xyz_down
            offset = offset_down
        return feats_stack, xyz_stack, offset_stack

    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        return self.forward_seg_feat(feats, xyz, offset, batch, neighbor_idx)


@MODELS.register_module()
class StratifiedDecoder(nn.Module):
    def __init__(self, channels,up_k, \
                 num_layers=4, **kwargs):
        super().__init__()
    ...
    def forward(self, feats_stack, xyz_stack, offset_stack):
        for i, upsample in enumerate(self.upsamples):
            feats, xyz, offset = upsample(feats, xyz, xyz_stack.pop(), offset, offset_stack.pop(),
