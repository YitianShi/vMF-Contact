    ...
    def forward(self,
        ...
        for i in range(self.blocks):
            ...
            if self.query_as_support:
                ...
                idx = None
        return query_xyz, torch.cat(new_features_list, dim=1), idx  # concatenate
...
class PointNet2Encoder(nn.Module):
    ...
    def forward_grasp_feat(self, xyz, features=None):
        if hasattr(xyz, 'keys'):
            xyz, features = xyz['pos'], xyz['x']
        if features is None:
            features = xyz.clone().transpose(1, 2).contiguous()
        ...
        if self.stem_aggr:
            features = self.stem(xyz, xyz, features)
            # _, features = self.stem_sa(xyz, features, xyz)
        l_xyz, l_features, l_idx = [xyz], [features], [None]
        for i in range(len(self.SA_modules)):
            # query_xyz is None. query for neighbors
            li_xyz, li_features, li_idx = self.SA_modules[i](l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
            l_features.append(li_features)
            l_idx.append(li_idx)
        return l_xyz, l_features, l_idx
