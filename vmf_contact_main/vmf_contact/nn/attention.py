...
class RelativeCrossAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        attn_output, attention_weights = self.multihead_attn(
            ...
            key=context,
            value=context,
            rotary_pe=None if query_pos is None else (query_pos, context_pos),
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(
        self, query, diff_ts=None, query_pos=None, context_pos=None, pad_mask=None
    ):
        if diff_ts is not None:
            adaln_query = self.adaln(query, diff_ts)
        else:
...
class FFWRelativeCrossAttentionModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            query, attention = self.attn_layers[i](
                query,
                context,
                diff_ts,
                query_pos,
...
class FFWRelativeSelfCrossAttentionModule(nn.Module):
    ...
    def __init__(
        ...
        self.num_layers = num_cross_attn_layers
        ...
        for _ in range(num_cross_attn_layers):
            ...
            if self_attn:
                ...
                self.self_attn_layers.append(None)
            self.ffw_layers.append(
                FeedforwardLayer(embedding_dim, embedding_dim, use_adaln=use_adaln)
            )
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            ...
            # Self attend first
            if self.self_attn_layers[i] is not None:
                query = self.self_attn_layers[i](
                    query, query, diff_ts, query_pos, query_pos
                )[0]

            # Cross attend to the context next
            query, attention = self.cross_attn_layers[i](
                query,
                context,
                diff_ts,
                cur_query_pos,
        ...
        return output, attention_weights if need_attention_weights else None
