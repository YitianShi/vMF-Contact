...
from .transforms import quaternion_apply, quaternion_invert, multiply_se3, axis_angle_to_quaternion, quaternion_to_axis_angle, quaternion_multiply
...
def se3_isotropic_gaussian_score(T: torch.Tensor, 
    ...
    lin_score = quaternion_apply(quaternion_invert(q), lin_score)    
    ang_score = igso3_score(q=q, eps=eps)
    
    return ang_score, lin_score

@torch.jit.script
def so3_isotropic_gaussian_score(T: torch.Tensor, 
                                 eps: Union[float, torch.Tensor], 
                                 std: Union[float, torch.Tensor]) -> Union[float, torch.Tensor]:
    q = T
    ang_score = igso3_score(q=q, eps=eps)
...
def adjoint_se3_score(T_ref: torch.Tensor, ang_score: torch.Tensor, lin_score: torch.Tensor) -> SE3_SCORE_TYPE:
    ...
    ang_score = quaternion_apply(T_ref[..., :4], ang_score)
    lin_score = torch.linalg.cross(T_ref[...,4:], ang_score, dim=-1) + quaternion_apply(T_ref[..., :4], lin_score)
...
def adjoint_isotropic_se3_score(x_ref: torch.Tensor, ang_score: torch.Tensor, lin_score: torch.Tensor) -> SE3_SCORE_TYPE:
    ...
    lin_score = torch.linalg.cross(x_ref, ang_score, dim=-1) + lin_score
...
def adjoint_inv_tr_se3_score(T_ref: torch.Tensor, ang_score: torch.Tensor, lin_score: torch.Tensor) -> SE3_SCORE_TYPE:
    ...
    lin_score = quaternion_apply(T_ref[..., :4], lin_score)
    ang_score = quaternion_apply(T_ref[..., :4], ang_score) + torch.linalg.cross(T_ref[...,4:], lin_score, dim=-1)
...
def adjoint_inv_tr_isotropic_se3_score(x_ref: torch.Tensor, ang_score: torch.Tensor, lin_score: torch.Tensor) -> SE3_SCORE_TYPE:
    ...
    ang_score = ang_score + torch.linalg.cross(x_ref, lin_score, dim=-1)
...
def sample_isotropic_se3_gaussian(eps: Union[float, torch.Tensor], std: Union[float, torch.Tensor], N: int = 1, dtype: Optional[torch.dtype] = torch.float64, device: Optional[Union[str, torch.device]] = None) -> torch.Tensor:
    x = torch.randn(N, 3, device=device, dtype=dtype) * std.unsqueeze(-1)
...
def diffuse_isotropic_se3(T0: torch.Tensor, 
    ...
    if x_ref is not None:
        delta_T = torch.cat([delta_T[...,:4],
                             delta_T[...,4:] + x_ref - quaternion_apply(delta_T[...,:4], x_ref)
                            ], dim=-1)        # shape: (nT, 7)

    T = multiply_se3(T0, delta_T) # shape: (nT, 7)
...
def diffuse_isotropic_se3_batched(T0: torch.Tensor, 
    ...
    if x_ref is not None:
        delta_T = torch.cat([delta_T[...,:4],
                             delta_T[...,4:] + x_ref.unsqueeze(-2) - quaternion_apply(delta_T[...,:4], x_ref.unsqueeze(-2))
                            ], dim=-1)        # shape: (nXref, nT, 7)

    T = multiply_se3(T0.unsqueeze(-3), delta_T) # shape: (nXref, nT, 7)
...
def diffuse_isotropic_so3_batched(T0: torch.Tensor, 
                          eps: Union[float, torch.Tensor], 
                          std: Union[float, torch.Tensor], 
                          double_precision: bool = True) -> Tuple[torch.Tensor, 
                                                                  torch.Tensor, 
                                                                  SE3_SCORE_TYPE]:
    assert T0.ndim == 2 and T0.shape[-1] == 4  # T0: shape (nT, 7)
    ...
    delta_T = sample_igso3(eps=eps, N=len(T0), dtype=T0.dtype, device=T0.device)
    ang_score_ref = so3_isotropic_gaussian_score(T=delta_T, eps=eps, std=std)                   # shape: (nXref*nT, 3), (nXref*nT, 3)
    ang_score = ang_score_ref    # shape: (nXref*nT, 3), (nXref*nT, 3)
    ang_score = ang_score.view(*T0.shape[:-1], 3)   # shape: (nXref, nT, 3)     
    ang_score_ref = ang_score_ref.view(*T0.shape[:-1], 3)  # shape: (nXref, nT, 3)

    T = quaternion_multiply(T0, delta_T) # shape: (nXref, nT, 7)

    return (
        T.type(dtype=input_dtype),                  # shape: (nXref, nT, 7)
        delta_T.type(dtype=input_dtype),            # shape: (nXref, nT, 7)
        (ang_score.type(dtype=input_dtype)),  # shape: (nXref, nT, 3), (nXref, nT, 3),
        (ang_score_ref.type(dtype=input_dtype))   # shape: (nXref, nT, 3), (nXref, nT, 3),
    )     
...
#         return axis_angle_to_quaternion(torch.randn(N,3, device=device, dtype=dtype) * math.sqrt(2*eps))
...
#         angle: torch.Tensor = torch.norm(quaternion_to_axis_angle(standardize_quaternion(q)), dim=-1)
#         if eps <= self.small_eps_criteria:
