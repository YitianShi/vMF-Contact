...
class LearnedAbsolutePositionEncoding3Dv2(nn.Module):
    def __init__(self, embedding_dim, input_dim=3, norm="none"):
        super().__init__()
        ...
        self.absolute_pe_layer = nn.Sequential(
            nn.Conv1d(input_dim, embedding_dim*2, kernel_size=1),
            norm_tb[norm],
            nn.GELU(),
            nn.Conv1d(embedding_dim*2, embedding_dim*2, kernel_size=1)
        )
    ...
    def forward(self, xyz):
        ...
        Returns:
            absolute_pe: (B, N, embedding_dim) tensor of the absolute position encoding
        """
        bt, npoint, _ = xyz.shape
        return self.absolute_pe_layer(xyz.permute(0, 2, 1)).permute(0, 2, 1).view(bt, npoint, self.embedding_dim, -1)
